...
module.exports = router => {
  ...
  router.all('/schools/participants', (req, res, next) => {
    ...
    res.locals.hasAnyContactedParticipants = Object.values(participants).some((p) => p.status === 'Contacted' && p.role == "ECT"  )
    res.locals.hasAnyContactedParticipantsMentor = Object.values(participants).some((p) => p.status === 'Contacted' && p.role == "Mentor"  )
    res.locals.hasAnyCheckingParticipants = Object.values(participants).some((p) => p.status === 'Checking' && p.role == "ECT" )
    res.locals.hasAnyCheckingParticipantsMentor = Object.values(participants).some((p) => p.status === 'Checking' && p.role == "Mentor"  )
    res.locals.hasAnyCheckingQTSParticipants = Object.values(participants).some((p) => p.status === 'CheckingQTS' && p.role == "ECT" )
    res.locals.hasAnyCheckingQTSParticipantsmentor = Object.values(participants).some((p) => p.status === 'CheckingQTS' && p.role == "Mentor"  )
